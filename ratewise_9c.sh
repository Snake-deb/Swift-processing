...
do
    ...
    cdnrtv=$(jq --argjson a "$val" 'if .cdnr != null then [.cdnr[].nt[].itms[].itm_det | select(.rt==$a) | .txval // 0 ] | add // 0 else 0 end' $value)
    sum_ret $val $cdnrtv "tcdnr"
    cdnriamt=$(jq --argjson a "$val" 'if .cdnr != null then [.cdnr[].nt[].itms[].itm_det | select(.rt==$a) | .iamt // 0 ] | add // 0 else 0 end' $value)
    sum_ret $val $cdnriamt "tcdnriamt"
    cdnrcamt=$(jq --argjson a "$val" 'if .cdnr != null then [.cdnr[].nt[].itms[].itm_det | select(.rt==$a) | .camt // 0 ] | add // 0 else 0 end' $value)
    sum_ret $val $cdnrcamt "tcdnrcamt"
    cdnurtv=$(jq --argjson a "$val" 'if .cdnur != null then [.cdnur[].itms[].itm_det | select(.rt==$a) | .txval // 0] | add //0 else 0 end' $value)
    sum_ret $val $cdnurtv "tcdnur"
    cdnuriamt=$(jq --argjson a "$val" 'if .cdnur != null then [.cdnur[].itms[].itm_det | select(.rt==$a) | .iamt // 0] | add //0 else 0 end' $value)
    sum_ret $val $cdnuriamt "tcdnuriamt"
    cdnurcamt=$(jq --argjson a "$val" 'if .cdnur != null then [.cdnur[].itms[].itm_det | select(.rt==$a) | .camt // 0] | add //0 else 0 end' $value)
    sum_ret $val $cdnurcamt "tcdnurcamt"
done 
done
#echo $txv5 $txvret5
count=0
names='tb2b tb2bcamt tb2bcamt tb2biamt tb2cl tb2clcamt tb2clcamt tb2cliamt tb2cs tb2cscamt tb2cscamt tb2csiamt texp texpcamt texpcamt texpiamt tcdnr tcdnrcamt tcdnrcamt tcdnriamt tcdnur tcdnurcamt tcdnurcamt tcdnuriamt'
tab='\t\t\t'
ret='ret'
function print {
echo 
...
echo "total | $ttax | $tcamt | $tcamt | $tiamt"
}
